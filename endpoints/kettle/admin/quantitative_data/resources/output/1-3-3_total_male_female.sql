SET @sectorId = (SELECT g.id FROM split_gender g INNER JOIN translation t ON g.literal_id=t.literal_id WHERE t.text="Total");
SET @nutsId = (SELECT id FROM nuts WHERE country_code="AT");
INSERT INTO profile (nuts_id, gender_id) VALUE (@nutsId, @genderId);

SET @sectorId = (SELECT g.id FROM split_gender g INNER JOIN translation t ON g.literal_id=t.literal_id WHERE t.text="Male");
SET @nutsId = (SELECT id FROM nuts WHERE country_code="AT");
INSERT INTO profile (nuts_id, gender_id) VALUE (@nutsId, @genderId);

SET @sectorId = (SELECT g.id FROM split_gender g INNER JOIN translation t ON g.literal_id=t.literal_id WHERE t.text="Female");
SET @nutsId = (SELECT id FROM nuts WHERE country_code="AT");
INSERT INTO profile (nuts_id, gender_id) VALUE (@nutsId, @genderId);
