SET @answerId = (SELECT a.id FROM split_answe a INNER JOIN translation t ON a.literal_id=t.literal_id WHERE t.text="At least 1/4 of the time");
SET @nutsId = (SELECT id FROM nuts WHERE country_code="AT");
INSERT INTO profile (nuts_id, answer_id) VALUE (@nutsId, @answerId);

SET @answerId = (SELECT a.id FROM split_answe a INNER JOIN translation t ON a.literal_id=t.literal_id WHERE t.text="Less than 1/4 of the time");
SET @nutsId = (SELECT id FROM nuts WHERE country_code="AT");
INSERT INTO profile (nuts_id, answer_id) VALUE (@nutsId, @answerId);
