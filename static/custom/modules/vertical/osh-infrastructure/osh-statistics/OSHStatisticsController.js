/**
 * @ngdoc controller
 * @name barometer.osh-infrastructure-osh-statistics.controller:OSHStatisticsController
 * @requires $scope
...
define(function (require) {
  ...
  function controller($scope, $stateParams, $state, configService, $log, $document,dataService, $window, $sce, $compile, $timeout, $rootScope) {
    ...
    |                                DATA LOAD                                     |
    |******************************************************************************/
      dataService.getAllMatrixStatistics(initialFilter).then(function (data) {
        $log.debug('getAllMatrixStatistics');
        //$log.warn(data);
      ...
      dataService.getMatrixStatisticsCountries().then(function (data) {
        ...
        data.data.resultset.map(function (elem) {
          ...
          if(elem[1] == $scope.pCountry){
            ...
            var html = '<span class="selected-tag" id="country'+elem[1]+'" data-ng-click="deleteTag($event)">'+'('+elem[1]+') '+$scope.i18nLiterals['L'+elem[0]] + '</span>';
        ...
        if (countryHasData == false && $scope.pCountry != null)
        {
          ...
          $scope.pCountry = "AT";
          $scope.searchParams.countries.push("39");
    ...
    |******************************************************************************/
      ...
      /**
       * @ngdoc method
       * @name ng.controller:EUCategoryResponseController#toggleCategoryClick
       * @methodOf barometer.osh-authorities.controller:EUCategoriesResponseController
       * @description
       * Function launched when clicking an Category Filter
       */
      $scope.toggleCategoryClick = function ($event) {

        var check1 = $('#category-filter-1:checked').length > 0;
        var check2 = $('#category-filter-2:checked').length > 0;
        var check3 = $('#category-filter-3:checked').length > 0;

        /*if(!check1) {
          $scope.searchParams.categories.filter1=0;
        }
        if(!check2) {
          $scope.searchParams.categories.filter2=0;
        }
        if(!check3) {
          $scope.searchParams.categories.filter3=0;
        }*/
        ...
        if(check1) {
          ...
          $scope.searchParams.categories.filter1=0;
          angular.element('span#categoryFilter1').remove();
        }
        ...
        if(check2) {
          ...
          angular.element('span#categoryFilter2').remove();
          $scope.searchParams.categories.filter2=0;
        }
        ...
        if(check3) {
          ...
          angular.element('span#categoryFilter3').remove();
          $scope.searchParams.categories.filter3=0;
        }
      ...
      /**
       * @ngdoc method
       * @name ng.controller:EUChallengesResponseController#confirmCategorySelection
       * @param {$event} $event from the browser
       * @methodOf barometer.osh-authorities.controller:EUChallengesResponseController
       * @description
       * Function launched when clicking confirm button of Ccategories Select
       */
      $scope.confirmCategorySelection = function($event){
        var check1 = $('#category-filter-1:checked').length > 0;
        var check2 = $('#category-filter-2:checked').length > 0;
        var check3 = $('#category-filter-3:checked').length > 0;
      ...
      $scope.deleteTag = function($event){
        ...
        }else if($event.target.id == 'categoryFilter1'){
          quitChecked = angular.element('.filter--dropdown--options #category-filter-1');
          $scope.searchParams.categories.filter1=0;
        }else if($event.target.id == 'categoryFilter2'){
          quitChecked = angular.element('.filter--dropdown--options #category-filter-2');
          $scope.searchParams.categories.filter2=0;
        }else if($event.target.id == 'categoryFilter3'){
          quitChecked = angular.element('.filter--dropdown--options #category-filter-3');
          $scope.searchParams.categories.filter3=0;
        }
      ...
      function search($event,filter) {
        ...
        dataService.getStatisticsFiltersData($scope.searchText, $scope.searchParams.categories, $scope.searchParams.countries)
          .then(function (data) {
            ...
            $state.transitionTo('osh-statistics', {pCountry:0}, {notify: false});
